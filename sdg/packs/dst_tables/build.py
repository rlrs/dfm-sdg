...
# DST articles start with a publication date line, e.g. "22. april 2026\n\n".
# Strip it so the target is clean article text rather than a dated press release.
_DATE_PREFIX_RE = re.compile(r"^\d{1,2}\.\s+\w+\s+\d{4}\s*\n+")

# Cycling through these seeds diversifies prompt structure and placement of [table].
# Each seed describes a style abstractly without prescribing exact wording.
_STYLE_SEEDS = [
    "Stil: direkte kommando, tabel placeret sidst i anmodningen.",
    "Stil: anmodning om at beskrive tendenser og udviklinger, tabel placeret sidst.",
    "Stil: sporgende tone, tabel placeret sidst.",
    "Stil: uformel, kontekstsaettende indledning om at have data, tabel placeret sidst.",
    "Stil: tabel placeret forst med [table], AFSLUT med en kort sætning der beder om en nyhedsartikel baseret på tallene.",
    "Stil: tabel placeret forst med [table], AFSLUT med en kort uformel sætning der beder om at beskrive eller analysere dataene.",
    "Stil: tabel placeret midt i anmodningen med kontekst pa begge sider.",
    "Stil: faglig og formel tone, simpel anmodning om en statistisk artikel, tabel sidst.",
    "Stil: anmodning der fremhæver et bestemt tema eller en bestemt vinkel, tabel sidst.",
    "Stil: journalistisk vinkel med klar nyhedsvinkel efterspurgt, anmodning forst, tabel sidst.",
    "Stil: forklarende tone rettet mod en ikke-ekspert, anmodning forst, tabel sidst.",
    "Stil: meget kort og direkte, minimal tekst udover selve tabellen.",
]

_SYSTEM_PROMPT_BASE = """\
Du er en assistent der laver træningsdata til sprogmodeller.

Din opgave er at skrive en naturlig brugeranmodning, der beder en AI om at skrive en statistisk \
artikel baseret på tabeldata fra Danmarks Statistik.

Anmodningen SKAL indeholde PRÆCIS teksten [table] det sted, hvor tabeldataene skal indsættes. \
Du må IKKE inkludere selve tabeldataene - brug udelukkende [table] som placeholder.

Skriv KUN selve brugeranmodningen. Ingen forklaringer, ingen overskrifter, ingen anforselstegn \
omkring svaret.

Krav:
- Anmodningen skal være på dansk
- Anmodningen skal lyde som noget en rigtig bruger ville skrive
- Anmodningen skal indeholde [table]
- Anmodningen skal bede om en artikel, beskrivelse eller analyse baseret på tabeldataene
- Anmodningen må IKKE referere til "ovenstående artikel" eller lignende - brugeren har ikke \
set artiklen, kun tabellen
- Undgå at starte med "Du er" eller rollebeskrivelser
- Anmodningen må IKKE specificere præcise formateringskrav, hvilke afsnit artiklen skal \
indeholde, eller andre strukturkrav - bed blot om en artikel eller analyse baseret på dataene
- Anmodningen må IKKE angive et bestemt antal ord, sætninger eller en bestemt længde
- Anmodningen må IKKE specificere hvilke emner, sektorer eller aspekter artiklen skal dække - \
bed blot om en artikel baseret på dataene
- Anmodningen skal ALTID slutte med en anmodning eller et spørgsmål - aldrig med en tabel\
"""
...
def publish(run_id_or_path: str, out_dir: str | None = None) -> dict[str, Any]:
    ...
    failure_summary = _load_or_compute(
        outputs_dir / "failure_summary.json", common_eval.summarize_failures(rows)
    )
...
def _record_to_article(
    ...
    tables_md_field = source.get("tables_md_field", "inline_tables_md")
    tables_md = str(record.get(tables_md_field) or "").strip()
...
async def _generate_row(
    *,
    ...
    if personas:
        persona = personas[(article["row_index"] * 7) % len(personas)]

    tables_md = article["tables_md"]
    messages = _instruction_messages(article, style_seed=style_seed, persona=persona)
    raw_prompt = await _generate_prompt(writer, messages, temperature=temperature)
    cleaned = _clean_generated_prompt(raw_prompt)

    if "[table]" not in cleaned:
        raise ValueError(f"generated prompt missing [table] placeholder: {cleaned!r}")

    table_idx = cleaned.index("[table]")
    before_table = cleaned[:table_idx].strip()
    after_table = cleaned[table_idx + len("[table]"):].strip()
    if not before_table and not after_table:
        raise ValueError(f"generated prompt contains only [table] with no request text: {cleaned!r}")

    # Strip orphaned connector words that end up before [table] after sentence punctuation,
    # e.g. "...Statistik? Brug [table]" → "...Statistik?\n\n[table]"
    cleaned = re.sub(r"(?<=[.!?:])\s+\w+\s+\[table\]", "\n\n[table]", cleaned)
    # Ensure the table is always on its own paragraph, regardless of how the LLM placed [table].
    # Only add newlines if [table] isn't already at the start of a line.
    normalized = re.sub(r"(?<!\n)[ \t]*\[table\]", "\n\n[table]", cleaned).strip()
    final_prompt = normalized.replace("[table]", tables_md)

    return {
        "id": f"{PACK}-{article['row_index']:06d}",
        "prompt": final_prompt,
        "target": article["text"],
        "sources": [
            _drop_none(
        ...
        "meta": _drop_none(
            {
                ...
                "table_chars": len(tables_md),
                "style_seed_index": article["row_index"] % len(_STYLE_SEEDS),
            }
        ),
    }
...
async def _generate_row_result(
    *,
    ...
    try:
        ...
        return {
            "failure": {
                "id": f"{PACK}-failure-{article['row_index']:06d}",
                "source_id": article["source_id"],
                "title": article["title"],
                "url": article["url"],
                "error_type": error.__class__.__name__,
...
def _instruction_messages(
    ...
    *,
    ...
    if persona:
        system_content += f"\n\nSkriv som om det er denne bruger der stiller sporgsmaalet:\n{persona}"

    user_lines = [style_seed, ""]
    if article["title"]:
        user_lines.append(f"Titel: {article['title']}")
    user_lines.append("")
    user_lines.append("Artikel:")
    user_lines.append(article["text"])
    user_lines.append("")
    n_tables = _count_tables(article["tables_md"])
    table_ref = "tabellen" if n_tables == 1 else f"de {n_tables} tabeller"
    user_lines.append(
        f"Skriv en brugeranmodning der ville fa en AI til at skrive en lignende statistisk "
        f"artikel baseret på {table_ref}. "
        "Husk at inkludere [table] som placeholder for tabeldataene - brug kun ét [table] "
        "uanset antallet af tabeller."
    )
...
def _count_tables(tables_md: str) -> int:
    """Count the number of markdown tables by counting header separator rows."""
    return len(re.findall(r"^\|[\s\-:|]+\|$", tables_md, re.MULTILINE))
