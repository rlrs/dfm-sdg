...
def select_clues(solution: tuple[str, ...], rng: Random, recipe: dict[str, Any]) -> tuple[Clue, ...]:
    ...
    selected: list[Clue] = []
    remaining_solutions = list(permutations(tuple(sorted(solution))))
    ...
    for kind in pending_required:
        extra = _best_extra_clue_of_kind(
            kind,
            candidates,
            selected,
            remaining_solutions,
        ...
        assert extra is not None, f"missing required clue kind: {kind}"
...
def _build_candidates(solution: tuple[str, ...], allowed_kinds: tuple[str, ...]) -> list[Clue]:
    ...
    positions = {name: index for index, name in enumerate(solution)}
    ...
    if "not_slot" in allowed:
        for index, name in enumerate(solution, start=1):
            for other_slot in range(1, len(solution) + 1):
                if other_slot == index:
                    continue
                candidates.append(Clue("not_slot", name, slot=other_slot))
    if "at_end" in allowed:
        for name, index in positions.items():
            if index in {0, last_index}:
                candidates.append(Clue("at_end", name))
    if "not_at_end" in allowed:
        for name, index in positions.items():
            if index not in {0, last_index}:
                candidates.append(Clue("not_at_end", name))
    if "middle" in allowed and len(solution) % 2 == 1:
        candidates.append(Clue("middle", solution[middle_index]))
    ...
    if "not_adjacent" in allowed:
        for left_index, left in enumerate(solution):
            for right in solution[left_index + 1 :]:
                if abs(left_index - positions[right]) > 1:
...
def _best_clue_of_kind(
    ...
    for clue in candidates:
        ...
        narrowed = _filter_orders(remaining_solutions, clue)
...
def _best_extra_clue_of_kind(
    ...
    for clue in candidates:
        ...
        if _exceeds_kind_limit(selected, clue, max_kind_counts):
            continue
        if not _filter_orders(remaining_solutions, clue):
            continue
        return clue
...
def _clue_holds(order: tuple[str, ...], clue: Clue) -> bool:
    ...
    if clue.kind == "not_slot":
        assert clue.slot is not None, "not_slot clue requires slot"
        return first_position != clue.slot - 1
    if clue.kind == "at_end":
        return first_position in {0, last_index}
    if clue.kind == "not_at_end":
        return first_position not in {0, last_index}
    if clue.kind == "middle":
        assert len(order) % 2 == 1, "middle clue requires odd-sized order"
        return first_position == len(order) // 2
...
def _filter_orders(
    ...
    return [order for order in orders if _clue_holds(order, clue)]
