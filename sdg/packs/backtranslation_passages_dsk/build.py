...
PACK = "backtranslation_passages_dsk"
            ...
            "sources": [
                {
                    ...
                    "content_type": entry["generation"].get("content_type", "news"),
                }
    ...
    *,
    ...
    for source_index, entry in enumerate(entries):
        ...
        for record in common_sources.iter_source_records(source):
            ...
            for passage in _iter_chunks(article, source, min_chars=min_article_chars):
        ...
        source_summaries.append(
            {
                "key": entry["key"],
                "content_type": generation.get("content_type", "news"),
                "scanned_rows": source_scanned_rows,
...
def _iter_chunks(article: dict[str, Any], source: dict[str, Any], *, min_chars: int):
    for i, passage_text in enumerate(_extract_passages(article["text"], min_chars=min_chars)):
...
def _score_paragraph_web(p: str) -> float:
    """Score a paragraph for web content (news and promo). Less strict than academic scoring."""
    if len(p) < 80:
        return 0.0
...
def _instruction_messages_news(
    ...
    *,
    ...
    system_lines = [
        ...
        "Undgå fraser som 'som en', 'ligesom' eller lignende billedsprog.",
        ...
        "'nedenstående dokument' eller lignende. "
        "Prompten skal beskrive hvad brugeren ønsker skrevet, ikke hvad brugeren ønsker præsenteret.",
        "Prompten må IKKE bede om referencer til bestemte navngivne artikler, studier eller "
        "specifikke hændelser der kun kendes fra den vedlagte tekst.",
    ]
    ...
    user_content = (
        "\n".join(user_lines)
        + "\n\n---\nSkriv en prompt der ville få en AI til at skrive ovenstående nyhedsartikel eller kommentar."
    )
    if style_block:
        user_content += f"\n\n{style_block}"
...
def _instruction_messages_promo(
    article: dict[str, Any],
    *,
    persona: str | None = None,
    prompt_length: str = "medium",
    ...
    system_lines = [
        ...
        "Undgå kreative metaforer, slogans og reklamesprog i prompten.",
        "Undgå fraser som 'som en', 'ligesom' eller lignende billedsprog.",
        "Prompten skal bede om hjemmesidetekst, virksomhedsportrættering, markedsføring eller "
        "organisationsbeskrivelse — IKKE om at analysere en bestemt virksomhed eller organisation.",
        "Prompten skal fremstå som en naturlig forespørgsel fra en person der har brug for tekst "
        "om et produkt, en virksomhed, en service eller en organisation.",
        "Prompten skal være selvstændig og må IKKE referere til specifikke dokumenter eller tekster "
        "der ikke er vedlagt — ingen 'ovenstående tekst', 'nedenstående dokument' eller lignende.",
    ]
    ...
    user_content = (
        "\n".join(user_lines)
        + "\n\n---\nSkriv en prompt der ville få en AI til at skrive ovenstående tekst som hjemmesidetekst, markedsføring eller virksomhedsbeskrivelse."
    )
    if style_block:
        user_content += f"\n\n{style_block}"
...
async def _generate_row(
    *,
    ...
    content_type = str(article.get("content_type", "news"))
    if content_type == "promo":
        messages = _instruction_messages_promo(article, persona=persona, prompt_length=prompt_length)
    else:
        messages = _instruction_messages_news(article, persona=persona, prompt_length=prompt_length)
...
def _clean_generated_prompt(value: str) -> str:
    ...
    # Extract quoted prompt from **"..."** wrapping
    if text.startswith('**"'):
    ...
    if re.search(r"\b(ligesom|som\s+om|et\s+strejf\s+af)\b", text, re.IGNORECASE):
        raise ValueError(f"metaphor-style prompt rejected: {text[:120]!r}")
    if re.search(
        r"\bsom\s+en\s+(gammel\s+dame|kollega|ven|kunstner|poet|historie|skitse|skitsetegning)\b",
        text,
        re.IGNORECASE,
    ):
        raise ValueError(f"metaphor-style prompt rejected: {text[:120]!r}")
...
def _looks_like_user_request(text: str) -> bool:
    ...
    return bool(
        re.search(
            r"\b(kan\s+du|kan\s+vi|skriv|lav|udarbejd|hjælp|giv|forklar|opsummer|formuler|foreslå|generer|beskriv|oversæt)\b",
            text,
            re.IGNORECASE,
        )
    )
...
def _contains_shouting_token(text: str) -> bool:
    allowed = {"AI", "USA", "ODEON", "F2", "NIS2", "CITES", "IDA", "HOFOR"}
...
def _source_entries(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    raw_key = source.get("name") or source.get("dataset") or source.get("path")
    return [{"key": _normalize_source_key(raw_key, index=0), "source": source, "generation": generation}]
